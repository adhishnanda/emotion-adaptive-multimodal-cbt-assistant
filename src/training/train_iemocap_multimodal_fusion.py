...
def multimodal_collate_fn(batch: List[Dict], tokenizer, sample_rate: int = 16000, n_mfcc: int = 40, device: torch.device = None):
    ...
    # Extract MFCC from audio waveforms
    mfcc_features = []
    ...
    if len(mfcc_features) > 0:
        ...
        audio_mfcc = torch.stack(padded_mfcc)  # (batch, n_mfcc, time)
    else:
        audio_mfcc = None
...
def train_iemocap_fusion():
    ...
    fusion_cfg = raw_cfg.get("iemocap_fusion", {})
    batch_size = fusion_cfg.get("batch_size", 4)
    learning_rate = fusion_cfg.get("learning_rate", 1e-4)
    num_epochs = fusion_cfg.get("num_epochs", 15)
