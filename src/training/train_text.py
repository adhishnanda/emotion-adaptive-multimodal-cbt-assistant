"""Full text-only training pipeline for DistilBERT emotion classifier."""
...
def train_text_model() -> None:
    """
    Main training function for text-only emotion classification.
    ...
    # Initialize logger
    logger = get_logger("text_training")
    ...
    best_model_path = model_save_dir / "distilbert_best.pt"
    ...
    for epoch in range(cfg.text_model.num_epochs):
        ...
        # Evaluate
        val_loss = evaluate(model=model, val_loader=val_loader, device=cfg.device.device)
